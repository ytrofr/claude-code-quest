        ...
        "demo": {
            "quests": [
                ...
                },
                {
                    "id": "done-old-quest",
                    "name": "Done Quest",
                    "desc": "Completed quest about variant engine and apollo long ago",
                    "status": "done",
                    "tags": ["done"],
                },
            ]
...
class TestDecideAction(unittest.TestCase):
    ...
    def test_generic_continuation_suggests_not_rebinds(self):
        ...
        self.assertEqual(r["top"], "nova-marketplace-landing")
    ...
    def test_conflict_guard_user_disagrees_with_context(self):
        ...
        # User prompt has nova → must NOT rebind to mobile-layout
        if r["action"] == "rebind":
            self.assertNotEqual(r["top"], "mobile-layout-iframe-cache",
                                "Conflict guard failed: rebound to context-favorite while user said nova")
    ...
    def test_locked_and_done_quests_never_picked(self):
        ...
        r = scorer.decide_action("done quest apollo variant engine ago", "", self.docs, self.idf)
